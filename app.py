...
@app.get("/mem/add")
def mem_add(mb: int = Query(100), chunk: int = Query(CHUNK_MB_DEFAULT)):
    """追加配額：/mem/add?mb=300&chunk=8"""
    if mb <= 0 or chunk <= 0:
        return JSONResponse(content={"error": "mb/chunk 必須 > 0"}, status_code=400)
    if mb > 4096:
        return JSONResponse(content={"error": "單次追加上限 4096MB，可自行放寬"}, status_code=400)
    ...
    with _mem_lock:
        ...
        while remain > 0 and _chunks:
            ...
            if not group:
                _chunks.pop()
        total = _current_mb()
    return {"ok": True, "freed_request_mb": mb, "total_mb": total}
